...
LDheatmap2 <- function(
    ...
    if (color[1] == "blueToRed") {
        color = rainbow(20, start = 4 / 6, end = 0, s = .7)[20:1]
    }
    if (newpage) {
        grid::grid.newpage()
    }
    ...
    ImageRect <-
        makeImageRect2(dim(LDmatrix)[1], dim(LDmatrix)[2], colcut, name = "heatmap", byrow)

    # Controls text placement
    ImageText <- NULL
    if (text) {
        ImageText <-
            makeImageText2(dim(LDmatrix)[1], dim(LDmatrix)[2], round(imgLDmatrix, digits = 2), name = "heatmaptext")
    }
    title <-
    ...
    heatMap <-
        grid::gTree(children = grid::gList(ImageRect, ImageText, title), name = "heatMap")
    ...
    # Draw the Color Key
    if (colorLegend) {
        Key <- LDheatmapLegend.add2(color, LDmeasure, heatmapVP)
    } else {
        Key <- NULL
    }
...
preDrawDetails.ldheatmap2 <- function(x) {
    fontsize <-
...
postDrawDetails.ldheatmap2 <- function(x) {
    grid::popViewport()
}


preDrawDetails.symbols2 <- function(x) {
    fontsize <-
        grid::convertX(unit(1 / 20, "grobwidth", grid::rectGrob()), "points")
    grid::pushViewport(grid::viewport(gp = grid::gpar(fontsize = fontsize)))
}

postDrawDetails.symbols2 <- function(x) {
    grid::popViewport()
}
...
makeImageRect2 <- function(nrow, ncol, cols, name, byrow = TRUE) {
    xx <- (1:ncol) / ncol
    yy <- (1:nrow) / nrow
...
makeImageText2 <- function(nrow, ncol, cols, name, flip = FALSE) {
...
LDheatmapLegend.add2 <- function(color, LDmeasure, vp) {
    ImageRect <-
        makeImageRect2(2, length(color), cols = c(rep(NA, length(color)), color[length(color):1]), "colorKey")
    keyVP <-
        grid::viewport(
            x = 1.1,
            y = -.10,
...
LDheatmapMapNew.add2 <- function(
    ...
    if (add.map) {
        ...
        if (flip) {
            geneMapLocation <-
                (-geneMapLocation)
        } # geneMapLocation gets flipped, reflects the gene bar
        ...
        #            1 - 0.5 * geneMapLocation - 1 / (nsnps * 2))
        seq.x <- c(0.5 * geneMapLocation + 1 / (nsnps * 2), 1 + 0.5 * geneMapLocation + 1 / (nsnps * 2))
        seq.y <- c(-0.5 * geneMapLocation - 1 / (nsnps * 2), 1 - 0.5 * geneMapLocation - 1 / (nsnps * 2))
        diagonal <-
            grid::linesGrob(
                seq.x,
                seq.y,
        ...
        if (!missing(Chr) & !missing(gff) & add.map) {
            gene <- GenomicRanges::GRanges(Chr, IRanges::IRanges(start = min.dist, end = max.dist))
            ...
            for (p in seq_len(length(ps))) {
                ...
                xsp <-
                    c(seq.x[1] - 0.002, seq.x[1] + 0.002, seq.x[2] + 0.002, seq.x[2] - 0.002)
                ysp <-
                    c(seq.y[1] + 0.002, seq.y[1] - 0.002, seq.y[2] - 0.002, seq.y[2] + 0.002)
        ...
        regionx <- seq.x[1] +
            ((genetic.distances - min.dist) / total.dist) *
                (seq.x[2] -
                    seq.x[1])
        regiony <- seq.y[1] +
            ((genetic.distances - min.dist) / total.dist) *
        ...
        if (distances == "physical") {
            mapLabel <-
                paste("Physical Length:", round((total.dist / 1000), 1), "kb", sep = "")
        } else {
            mapLabel <-
        ...
        if (!is.null(SNP.name) && (any(ind != 0))) {
            if (flip) {
                ...
                title <-
                    grid::editGrob(title, y = unit(geneMapLabelY + name_gap + 0.05, "npc"))
                ...
                if ((!is.null(xs)) & (!is.null(ys))) {
                    gmodel <- grid::polygonGrob(
                        x = xs - name_gap,
                        y = ys + name_gap,
                        gp = grid::gpar(fill = color_gmodel, lty = 0),
                        vp = vp
                    )
                }
            }
        ...
        # if(add.map) end
        # ✅ 提前计算 name_gap，避免分支依赖问题
        length_SNP_name <- if (!is.null(SNP.name)) max(nchar(SNP.name)) else 1
        long_SNP_name <- paste(rep(8, length_SNP_name), collapse = "")
        name_gap <- grid::convertWidth(
            grid::grobWidth(grid::textGrob(long_SNP_name)),
            "npc",
            valueOnly = TRUE
        )

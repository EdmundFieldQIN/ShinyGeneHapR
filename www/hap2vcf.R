...
vcf2hap_data2 <- function(
  ...
  # ✅ 兼容不同版本
  if (!"Key" %in% colnames(gt)) {
    if ("VariantKey" %in% colnames(gt)) {
      gt$Key <- gt$VariantKey
    } else {
      gt$Key <- paste(gt$CHROM, gt$POS, sep = "_")
    }
  }
